...
QMap<QString, QString> CMidiOut::devices()
{
	return buildDevicesMap(false);
}

bool CMidiOut::connect(QString outDeviceId)
{
	if (fConnected)
		disconnect();
...
void CMidiOut::sendMsg(qint32 msg)
{
	if (!fConnected)
		return;
...
void CMidiOut::sendSysEx(const QByteArray &data)
{
	if (!fConnected)
		return;
...
struct NativeMidiInInstances {
    ...
    CMidiInternal::MidiInReceiveThread* receiveThread;
};

QMap<QString, QString> CMidiIn::devices()
{
	return buildDevicesMap(true);
}

bool CMidiIn::connect(QString inDeviceId)
{
	if (fConnected)
		disconnect();
...
CMidiInternal::MidiInReceiveThread::MidiInReceiveThread(CMidiIn* CMidiIn, NativeMidiInInstances* fMidiPtrs, QObject* parent)
    : QThread(parent), fMidiIn(CMidiIn), fMidiPtrs(fMidiPtrs)
{}

void CMidiInternal::MidiInReceiveThread::run()
{
