...
public:
    ...
    //! \c key which needs to be used with CMidiIn::connect, whereas the
    ...
    explicit CMidiIn(QObject *parent = nullptr);
    ~CMidiIn();

	//! \brief connect Connect to the MIDI input device specified by
	//! <tt>inDeviceId</tt>.
    //! \param inDeviceId The device ID, as returned by CMidiIn::devices.
    ...
    //! \brief deviceId Returns the device ID used with CMidiIn::connect.
